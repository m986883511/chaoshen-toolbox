...
class InstanceEvents(object):
    def __init__(self):
        self._events: typing.Optional[typing.Dict[str, _InstanceEvents]] = {}
    ...
    def prepare_for_instance_event(
        ...
        print('Preparing to wait for external event %(name)s-%(tag)s', {'name': name, 'tag': tag})
...
class ComputeVirtAPI(VirtAPI):
    def __init__(self, compute):
        ...
        self._compute: ComputeManager = compute
    ...
    def wait_for_instance_event(self, instance, event_names, deadline=300, error_callback=None):
        ...
        try:
            ...
            print(f'Timeout waiting for {_events} for instance with vm_state {vm_state} and '
                  f'task_state {task_state}. Event states are: {event_states}')

            raise

        elapsed = sw.elapsed()
        event_names = ','.join(x[0] for x in event_names)
        print(f'Instance event wait completed in {elapsed} seconds for {event_names}')
...
if __name__ == '__main__':
    def err_callback(event_name, instance):
        print(f'err_callback: {event_name}')
        return True
